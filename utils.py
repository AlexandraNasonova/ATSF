    ...
    for t in range(0, T):
        if not math.isnan(x[t]):
            ...
            if (t - t0 + 1) < AdaptationPeriod:
                y = y * (1 - alpha * (t - t0 + 1) / (AdaptationPeriod)) + alpha * (t - t0 + 1) / (AdaptationPeriod) * x[
                    t]
...
def HoltExponentialSmoothing(x, h, Params):
    ...
    for t in range(T):
        if not math.isnan(x[t]):
            ...
            if (t - t0 + 1) < AdaptationPeriod:
                l = (1 - (1 - alpha) * (t - t0 + 1) / (AdaptationPeriod)) * x[t] + (1 - alpha) * (t - t0 + 1) / (
                    AdaptationPeriod) * (l + b)
                b = (1 - beta) * (t - t0 + 1) / (AdaptationPeriod) * (l - l_prev) + (1 - alpha) * (t - t0 + 1) / (
                    AdaptationPeriod) * b
            else:
...
def AdaptiveExponentialSmoothing(x, h, Params):
    ...
    for t in range(0, T):
        if not math.isnan(x[t]):
            ...
            if (t - t0 + 1) < AdaptationPeriod:
                y = y * (1 - alpha) * (t - t0 + 1) / (AdaptationPeriod) + (
                            1 - (1 - alpha) * (t - t0 + 1) / (AdaptationPeriod)) * x[t]
            else:
                y = y * (1 - alpha) + (alpha) * x[t]
        FORECAST[t + h] = y
    return FORECAST
